...
class ProxyTest {
	...
	private <M extends Reactable & Reflective> void proxyTest(M obj, M newObj) {
		...
		Assertions.assertEquals(TestData.TestEnum.GREEN, lastValue);
		td.setId(99);
		assertEquals("id", lastChanged);
		td.setColor(TestData.TestEnum.BLUE);
		Assertions.assertEquals(TestData.TestEnum.BLUE, lastValue);
	...
	void testForwardingToReactable() {
		...
		d1.set("id", 12);
		Assertions.assertEquals(12, d1.getId(), "Call to the reactive part of a Subject should be forwarded");
		d1.set("id", 15);
		Assertions.assertEquals(15, d1.getId(), "Call to the reactive part of a Subject should be forwarded");
