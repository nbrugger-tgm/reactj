...
public class EqualsAndHashcodeTest {
	...
	class ReactiveObjectTests{
		...
		DynamicContainer testReactiveObjects() {
			...
			ReactiveObject ro = new ReactiveObject(withHash);
	...
	class ReactiveProxyTests{
		@TestFactory
		DynamicContainer testProxySubjects() {
			RProxy.WithHashEquals withHash  = ReactiveProxy.createProxy(RProxy.WithHashEquals.class).getObject();
			RProxy.WithHashEquals withHash2 = ReactiveProxy.createProxy(RProxy.WithHashEquals.class).getObject();

			RProxy.WithoutHashEquals withOutHash = ReactiveProxy.createProxy(RProxy.WithoutHashEquals.class).getObject();
			RProxy.WithoutHashEquals withOutHash2 = ReactiveProxy.createProxy(RProxy.WithoutHashEquals.class).getObject();
	...
	private static <A, B, C, D> DynamicContainer generateTests(
		...
		Stream<DynamicNode> hashless =  Stream.of(
				...
				dynamicTest("content equality (reactive->base)",()->equalsTest(equalBaseWithoutHash, withOutHash)),
	...
	public static void hashTest(Object original, Object reactive, Object[] unique){
		...
		assertEquals(original.hashCode(),reactive.hashCode(),"Hash code of Original Object ("+original+") and reactive counterpart should be the same");
		...
		assertEquals(map.get(original),Integer.MAX_VALUE,"the unreactive version should act the same as a key as the reactive version");
	}
	...
	public static void serializeTest(Object o){
		assertDoesNotThrow(()->{
			...
			assertEquals(o.hashCode(),read.hashCode(),"Deserialized Object should have same hash code as original");
