...
public class ConsumerBuilder<T, O extends ExposedCallBuilder<O>> {
	private final O                rootBuilder;
	private final ConsumerGroup<T> consumer = new ConsumerGroup<>();

	public ConsumerBuilder(O rootBuilder, Consumer<T> consumer) {
	 ...
	 * @see ConsumerGroup
	 */
	public ConsumerBuilder<T, O> and(Consumer<T> consumer) {
		this.consumer.add(consumer);
		return this;
	}
	 ...
	 */
	public ExposedSourceBindingBuilder<T> with(Supplier<T> source) {
		return new SourceBindingBuilder<>(new ReactiveBinding<>(consumer, source), rootBuilder);
	}
	 ...
	 */
	public ExposedSourceBindingBuilder<T> withValue(T constant) {
	 ...
	 */
	public <F> ConvertingConsumerBuilder<F, O> with(Function<F, T> converter) {
